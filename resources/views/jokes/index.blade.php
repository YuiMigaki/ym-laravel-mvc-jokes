                        ...
                        <table class="min-w-full text-left text-sm font-light text-surface dark:text-white">
                            ...
                            @foreach($jokes as $joke)
                                <tr class="border-b border-zinc-300 dark:border-white/10">
                                    ...
                                    <td class="whitespace-nowrap px-6 py-4">{{ $joke->user ? $joke->user->getRoleNames()->first() : 'Not assigned a role'  }}</td>
                                    ...
                                    <td class="whitespace-nowrap px-6 py-4">
                                        <form action="{{ route('jokes.destroy', $joke) }}"
                                            ...
                                            @auth
                                                @if (auth()->user()->id === $joke->user_id || auth()->user()->hasRole('Superuser') || auth()->user()->hasRole('Staff') )
                                                    {{--Check if the current authenticated user is same as the joke's user_id--}}
