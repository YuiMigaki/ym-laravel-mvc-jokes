<x-guest-layout>
    <form method="POST" action="{{ route('register') }}">
        ...
        <div>
            ...
            <x-text-input id="family_name" class="block mt-1 w-full" type="text" name="family_name" :value="old('family_name')" required autofocus autocomplete="username" />
            <x-input-error :messages="$errors->get('family_name')" class="mt-2" />
        </div>
        <div>
            <x-input-label for="given_name" :value="__('Given Name')" />
            <x-text-input id="given_name" class="block mt-1 w-full" type="text" name="given_name" :value="old('given_name')" required autofocus autocomplete="username" />
